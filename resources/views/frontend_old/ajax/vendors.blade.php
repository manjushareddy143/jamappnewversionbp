<div class="modal-header">
 	<h4 class="modal-title">
 		{{ $vendor->vendor ? $vendor->vendor->full_name : '' }}
	</h4>
...
<div class="modal-body">
	<form id="bookingForm" method="POST">
		@csrf 
		<input type="hidden" name="service_id" value="{{ $vendor->service_id }}">
		<input type="hidden" name="category_id" value="{{ $vendor->category_id }}">
		<input type="hidden" name="provider_id" value="{{ $vendor->user_id }}">
		<div class="msg_login"></div>
		<div class="row">
			<div class="col-md-4">
				<div class="form-group">
					<label>{{ __('frontend.primay_service') }}</label>
					<input type="text" name="service_id" class="form-control" value="{{ $vendor->service ? $vendor->service->name : '' }}" disabled>
				</div>
			</div>
			<div class="col-md-4">
				<div class="form-group">
					<label>{{ __('frontend.secondry_service') }}</label>
					<input type="text" name="category_id" class="form-control" value="{{ $vendor->categories ? $vendor->categories->name : '' }}" disabled>
				</div>
			</div>
			<div class="col-md-4">
				<div class="form-group">
